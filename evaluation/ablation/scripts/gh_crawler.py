...
headers = {"Authorization": "token "} # updated token
...
def main():
	...
	with open(f'{LANGUAGE}-evaluate-repos.txt', 'w') as f:
...
def run_query(max_stars):
	...
	while end_cursor != "":
		...
		query = f"""
		{{
		  search(query: "language:{LANGUAGE} fork:false created:>2023-05-01 sort:stars stars:<{max_stars}", type: REPOSITORY, first: 100 {', after: "' + end_cursor + '"' if end_cursor else ''}) {{
			edges {{
			  node {{
				... on Repository {{
				  url
