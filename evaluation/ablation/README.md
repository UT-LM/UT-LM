...
## 5. Selected files
    From the statistical files python.csv obtained in step four, filter the data. Select the data where is_training_data=False, callee_num>=3, and single_test_num>=3, and manually move the corresponding failpairs files to the method_with_focal_context_manual_selected folder.

## 6. LLM serving via vLLM.
...
## 7. Generate new evaluate data from the method_with_focal_context_manual_selected folder for ablation experiments.
    for model in models:
...
## 8. Clean and extract code from generated answers
    for model in models:
        sh clean_answers.sh ../promptsOutput/python/{model}

## 9. Compile, test, and run coverage
    for model in models:
        Assume in python310 image
